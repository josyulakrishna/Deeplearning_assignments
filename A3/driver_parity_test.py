...
def main():
    ...
    train_loader = DataLoader(train, batch_size=80, shuffle=True, collate_fn=pad_collate)
    hidden_dim = 4
    model = ParityLSTM(1, maximum_training_sequence_length, hidden_dim)
...
class ParityLSTM(torch.nn.Module):
    ...
    def __init__(self, input_size, max_len,hidden_dim=64):
        super().__init__()
        self.hidden_dim = hidden_dim
        self.num_layers = 3
        self.max_len = max_len
        # input_size, hidden_size, num_layers
        self.rnn = nn.LSTM(input_size, hidden_dim, self.num_layers, batch_first=False, bidirectional=False) #, dropout=0.07)
    ...
    def forward(self, x, s):
        ...
        # self.num_layers,
        h0 = torch.zeros((self.num_layers, x.size(0), self.hidden_dim)).to(0)
        c0 = torch.zeros((self.num_layers, x.size(0), self.hidden_dim)).to(0)
        # h0 = torch.zeros((self.num_layers, 1, self.hidden_dim)).to(0)
        # c0 = torch.zeros((self.num_layers, 1, self.hidden_dim)).to(0)

        # originadriver_parity_working.pyl LSTM shape
        # print("input ", x.shape)
        ...
        si = torch.tensor(s) - 1
        rs = torch.arange(0, out_lstm.shape[1])
        ones_i_want = out_lstm[:, rs, si]
        ones_i_want = ones_i_want.view(ones_i_want.shape[1],self.max_len)
        # h0 = h0.view(-1, s)
        # print("output lstm ", out_lstm.size())
        # print("hidden lstm ", h0.size())
        # print("output lstm ", out_lstm.size())
        # print("ones i want lstm ", ones_i_want.size())
        out = self.lin(ones_i_want)
        # print("output lin ", out.size())
        # out = self.lin2(out)
        out = F.softmax(out)
        # print("output sm ", out.size())
        return out
...
def runParityExperiment(model, max_train_length):
    ...
    while k <= 20:
        ...
        val_loader = DataLoader(val, batch_size=80, shuffle=False, collate_fn=pad_collate)
...
def train_model(model, train_loader, epochs=200, lr=0.003):
